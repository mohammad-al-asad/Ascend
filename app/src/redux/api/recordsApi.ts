...
export interface EmergencyContacts {
  scs_on_call_phone: string | null;
  ptim_clinic_phone: string | null;
  ptim_clinic_hours: string | null;
  chaplain_hotline_phone: string | null;
  family_contact_note: string | null;
}

export interface RehabStatusActive {
  available: true;
  phase: string;
  phase_label: string;
...
export interface RehabStatusUnavailable {
  available: false;
}

export type RehabStatus = RehabStatusActive | RehabStatusUnavailable;
...
export const recordsApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getUploads: builder.query<
      ...
      { document_type?: string; search?: string } | void
    >({
      query: (params) => {
